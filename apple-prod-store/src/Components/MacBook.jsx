...
const MacBook = () => {
  ...
  const currentModel = macbookModels[selectedChip]
  const currentColor = currentModel.colors[selectedColor]

  const handleSelectedChip = (index) => {
    setSelectedChip(index)
    setSelectedColor(0) // Reset color selection when chip changes
  }
    ...
    <div className='flex flex-col items-center justify-around bg-white p-8 h-full'>
        ...
        <div className='flex'>
          ...
          macbookModels.map((model, index) => (
            <button key={index} className={`2xl:w-36 md:w-26 w-28 2xl:h-18 md:h-12 h-12 flex flex-col justify-evenly items-center border
            ...
            >
            <span className='2xl:text-lg xl:text-sm text-xs font-bold text-gray-800'>With {model.chip} chip</span>
            <span className='2xl:text-base xl:text-sm text-xs text-gray-600'>{model.price}</span>
        </button>
        ...
        <div className='flex flex-col 2xl:p-4 md:items-start items-center'>
          <div className='w-80 h-64 2xl:mb-0 -mb-8'>
            <img src={currentColor.img} alt={`MacBook Air with ${currentModel.chip}`} className='2xl:w-72 xl:w-52 w-48 h-full 2xl:mt-0 sm:mt-6 object-contain m-auto' />
            </div>
            ...
            <div className='flex space-x-2 mb-4 md:text-left justify-center md:justify-start'>
              {
                currentModel.colors.map((color, index) => (
                  <button key={index} style={{ backgroundColor: color.bg }}
                  className={`2xl:w-6 sm:w-5 2xl:h-6 sm:h-5 w-4 h-4 bg-[#2e3641] rounded-full border hover:border-blue-500 
            ...
            <img src={currentModel.icon} alt={`Apple ${currentModel.chip}`} className='w-12 h-12 2xl:mb-2 xl:mb-2' />
